    ...
    public partial class GrammarNode : ParseTree
    {
        ...
        private void ValidateCodeBlock(ParseTree tree, NonTerminalSymbol nts, ParseNode node)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                if (s == null)
                {
                    tree.Errors.Add(new ParseError("Variable $" + match.Groups["var"].Value + " cannot be matched.", 0x1016, node.Token.File, node.Token.Line, node.Token.StartPosition + match.Groups["var"].Index, node.Token.StartPosition + match.Groups["var"].Index, match.Groups["var"].Length));
